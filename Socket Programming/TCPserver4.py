...
def startContest(s, addr,thread_id):
    ...
    threadnum = 0
    print(f"Player {threadnum} - {username} connected")
    points = 0

    # Send # of questions
    s.sendall(str(len(questions)).encode())

    for question in questions:
        # print(question)
        question['msg'] = f"Thread {thread_id} - Current Points: {points}"
        print(question['msg'])
        ...
        if answer in valid_answers and answer is valid_answers[question['correct_answer']]:
            ...
            #print([question[answer]])
            s.sendall(msg.encode())
        print(f"Thread {thread_id} - Total points: {str(points)}")
...
while True:
    ...
    thread_id = uuid.uuid4() # generate a unique identifier for the thread
    threading.Thread(target = startContest, args = (s, addr, thread_id)).start()
